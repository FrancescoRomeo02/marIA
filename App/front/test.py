...
def send_question():
    ...
    if st.session_state['client_socket'] and st.session_state['question']:
        ...
        # Mostra la domanda e la risposta nella UI
        render_messages()
...
# Layout per la chat e il box della domanda
st.write("---")  # Separatore per maggiore chiarezza

# Sezione della chat
render_messages(st.session_state['chat_history'])

# Box di input per la domanda
question_input = st.text_input("Fai una domanda al chatbot:", key="question", value=st.session_state['question'], placeholder="Scrivi qui...")
...
if question_input:  # Controllo se l'input non è vuoto
    send_question()